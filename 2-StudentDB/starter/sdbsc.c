...
int get_student(int fd, int id, student_t *s){
    ...
    if (s == NULL){ // Make sure pointer is valid
        return ERR_DB_FILE;
    }
    ...
    if (lseek(fd, offset, SEEK_SET) < 0) {
        return ERR_DB_FILE; // Seek error
    }

    // Read the record at the calculated offset to *s
    int bytes_read = read(fd, s, sizeof(student_t));
    // Make sure it worked
    if (bytes_read < 0) { // Didn't work case
    ...
    if (memcmp(s, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0){
        return SRCH_NOT_FOUND; // Student not found case (returns -3)
    }

    return NO_ERROR; // Successfully retrieved
}
...
{
    student_t student;
    student_t check = {0}; // to check if student exists already
    int offset = id * sizeof(student_t);
    ...
    size_t bytes_read = read(fd, &check, sizeof(student_t));
    if (bytes_read < 0) { 
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE; 
    }

    if (memcmp(&check, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) { // Student can't already exist / already exists at that location
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    }


    if (validate_range(id, gpa) != NO_ERROR) { //validate ID and GPA
    ...
    size_t written = write(fd, &student, sizeof(student_t));

    printf(M_STD_ADDED, id); // Print student
    return NO_ERROR;
}
...
int del_student(int fd, int id){
    ...
    if (result == SRCH_NOT_FOUND) { // (SRCH_NOT_FOUND=-3)
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    } 

    // Overwrite the student record with EMPTY_STUDENT_RECORD
    int offset = id * sizeof(student_t);
    if (lseek(fd, offset, SEEK_SET) == -1 || write(fd, (const void*)&EMPTY_STUDENT_RECORD, sizeof(student_t)) != sizeof(student_t)) 
    {
        //the inclusion of "(const void*)&" before the empty record is necessary to function.
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE; // Write error
    }

    printf(M_STD_DEL_MSG, id);
    return NO_ERROR; // Successfully deleted
}
...
int count_db_records(int fd){
    ...
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
        if (memcmp(&student, (const void*)&EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            count++;
        }
    }
...
int print_db(int fd)
{
    ...
    return NO_ERROR; // Successfully printed
}

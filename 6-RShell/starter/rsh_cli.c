...
int start_client(char *server_ip, int port){
    ...
    printf("Attempting to connect to %s:%d\n", server_ip, port); // Debug message

    int attempts = 5; // Number of connection attempts before giving up
    while (attempts > 0) {
    ...
    close(cli_socket);
    return ERR_RDSH_CLIENT; // If connect() fails after multiple attempts, return error


    return cli_socket; // Return the client socket if successful
}
